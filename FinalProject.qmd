...
We extend this finding to hypothesize that individual student attributes may be correlated with household characteristics, thus impacting student learning outcomes in basic literacy and numeracy. Individual student attributes include beliefs, growth metrics, intrinsic academic motivation, and career aspirations. Additionally, we explore individual student schooling history to determine if that impacts student learning outcomes.
...
**Map 1** visualizes Pakistan with its provincial and district-level geographical boundaries. The red boundary indicates the Punjab province where the three sample districts and sample schools are located. Province is comparable to a state in Pakistan’s context.
...
**Map 2** zooms into the Punjab province and identifies the three sample districts. The northern district of Attock is highlighted in red. Faisalabad, highlighted in green, is the central district. Last, our third sample district, highlighted in blue, is in the southern part of Punjab.
...
**Map 3** captures the spread of private and public schools at the village level in the northern district of Attock. We focus on only one village for two reasons. First, the village is the appropriate administrative unit for sampling for data collection or intervention randomization. Second, the village serves as a competitive market for the booming public and private schools. Interestingly, we can see only one public primary school in the middle of 7 private schools. This is because the public system provides only one primary school per village, while the existence of private schools indicates parental demand for both access to education and better quality of education.

```{r}
attock <- pakistan_district_shape[pakistan_district_shape$districts == "Attock", ]

ggplot() +
  geom_sf(data = attock, fill = "lightgray", color = "black") +
  geom_point(data = sample_schools, aes(x = longitude, y = latitude), color = "red") +
  labs(title = "Map of Punjab Province (Sample Province) with Sample Districts Highlighted") +
  theme_minimal()
...
ASER-Pakistan 2019. (2020). Annual Status of Education Report. [aserpakistan.org/document/aser/2019/reports/national/ASER_National_2019.pdf](http://aserpakistan.org/document/aser/2019/reports/national/ASER_National_2019.pdf)
...
UNESCO. (2017). [unesco.org/news/617-million-children-and-adolescents-not-getting-minimum-reading-and-math](https://en.unesco.org/news/617-million-children-and-adolescents-not-getting-minimum-reading-and-math)
